    ...
    <a href="#" class="btn btn-primary" data-toggle="modal"
       data-target="#newAtdDialog" onclick="clearAtdInfo()">新建</a>
        <div class="btn-group">
            ...
            <button type="button" class="btn btn-default" onclick="outputInfo()"><span class="glyphicon glyphicon-export"></span> 导出</button>
        </div>
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <table class="table table-bordered table-striped" id="emp_table" style="text-align: center;font-size: 14px;">
                    <thead>
                    <tr>
                        <th width="82px" height="39px">编号</th>
                        <th width="111px">姓名</th>
                        <th width="189px">部门</th>
                        <th width="128px">迟到(次)</th>
                        <th width="129px">早退(次)</th>
                        <th width="129px">出差(天)</th>
                        <th width="129px">旷工(天)</th>
                        <th width="129px">请假(天)</th>
...
<div class="modal fade" id="importModal" role="dialog">
    <div class="modal-dialog" role="document" style="width: 85%;">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                <button type="button" class="btn btn-primary" data-dismiss="modal" onclick="inputAtdInfo()">导入</button>
            </div>
        </div>
    </div>
</div>
...
<script type="application/javascript">
    function inputAtdInfo() {
        var form = new FormData($("#file_form")[0]);
        $.ajax({
            url:"${pageContext.request.contextPath}/attend/upload",
            type:"post",
            data:form,
    ...
    function outputInfo() {
        window.location.href = "${pageContext.request.contextPath}/attend/download";
    }
</script>
            ...
            $.post("${pageContext.request.contextPath}/super/delete",
                ...
                function (data) {
                    if (data == "ok"){
                        ...
                        /*console.log("now_page "+now_page+"emp_total "+emp_total+"now_page_index "
                            +now_page_index+"page_total "+page_total);*/
                    }else{
                        alert("删除失败")
                    }
                }
