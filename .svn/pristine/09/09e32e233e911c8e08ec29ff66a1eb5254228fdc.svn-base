        ...
        <div class="col-lg-12">
            ...
            <div style="height: 50px;">
                ...
                <nav aria-label="Page navigation" style="margin-top: -35px;position: absolute;right:15px;">
                    <ul class="pagination" id = "shuaxin">
                        <li>
...
<script type="application/javascript">
    ...
    function each_emps(emps) {
        ...
        for (var i=0; i<emps.length; i++){
            ...
            if(emps[i]["level"]=="2"){
                table.rows[i+1].cells[7].innerHTML="<button type='button' class='btn btn-success btn-xs'>管理员</button>";
            }
            else if(emps[i]["level"]=="3"){
                table.rows[i+1].cells[7].innerHTML="<button type='button' class='btn btn-primary btn-xs'>职员</button>";
            }
            if(emps[i]["disable"] == "0"){
    ...
    function getEmpByPage(page) {
        ...
        if (search_state == 1){
            page_url = "${pageContext.request.contextPath }/super/getadmsg/"+page;
            data = {};
        }else if(search_state == 2){
            page_url = "${pageContext.request.contextPath }/super/tj_srch";
            data = eval("(" + "{dept:'" + dept+ "',gender:'" + gender + "',disable:'"+ disable +"',start_index:'" + page + "'}" +")")
        }else{
            page_url = "${pageContext.request.contextPath }/super/suad_mh_srch";
...
<script type="application/javascript">
    ...
    $(function () {
        $("#condition_srch").click(function () {
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/super/tj_srch",
                type:"get",
                data:{dept:dept, gender:gender, disable:disable, start_index:1,flag:2},
                dataType:"json",
                success:function (data) {
                    console.log(data);
                    $("#emp_total").text(data.total);
    ...
    $(function () {
        $("#mh_srch").click(function () {
            ...
            $.ajax({
                url:"${pageContext.request.contextPath}/super/suad_mh_srch",
